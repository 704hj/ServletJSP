...
public class OrderDAO {
	...
	public void insertOrder(OrderVO order, 
		...
		try {
			...
			pstmt2.setLong(10, order.getMem_num());
			pstmt2.executeUpdate();

			//주문상세정보
			sql = "INSERT INTO zorder_detail (detail_num,"
					+ "item_num,item_name,item_price,item_total,"
			...
			//상품의 재고수 차감
			sql = "UPDATE zitem SET quantity=quantity-? WHERE "
					+ "item_num = ?";
	...
	public int getOrderCount(String keyfield,
		...
		try {
			...
			sql = "SELECT COUNT(*) FROM zorder JOIN (SELECT order_num,"
					...
					+ "GROUP BY order_num) USING(order_num) JOIN zmember USING(mem_num) " + sub_sql;
	...
	public List<OrderVO> getListOrder(int start,int end,
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM ("
					...
					+ "LISTAGG(item_name,',') WITHIN GROUP "
					+ "(ORDER BY item_name) item_name FROM zorder_detail "
					+ "GROUP BY order_num) USING(order_num) JOIN zmember USING(mem_num) "
					+ sub_sql
		...
		try {
			...
			if(order.getStatus() == 5) {
				...
				for(int i=0;i<detailList.size();i++) {
					...
					//계속 추가혐 outOfMemory 발생, 1000개 단위로
					//executeBatch()
					if(i % 1000 == 0) {
						pstmt2.executeBatch();
					}
	...
	//사용자 - 주문 취소
	public void updateOrderCancel(long order_num)
	                                      throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		PreparedStatement pstmt2 = null;
		...
		try {
			...
			conn.setAutoCommit(false);
			//SQL문 작성
			sql = "UPDATE zorder SET status=5,"
