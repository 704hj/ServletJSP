...
public class BoardDAO {
	...
	public List<BoardVO> getList(int startRow, int endRow) throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM (SELECT * FROM svboard ORDER BY num DESC)a) WHERE rnum >= ? AND rnum <= ?";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
	...
	public void update(BoardVO boardVO)throws Exception {
        ...
        try {
            ...
            pstmt.setLong(6, boardVO.getNum());
	...
	//요청URL      흐름 제어            데이터 처리           UI
	//list.do-> DisparcherServlet -> 리스트 엑션        ->리스트           -> HTML전송
	//detail.do                   -> DetailAction    -> detail.jsp
	//writeForm.do                -> writeFormAction -> writeForm.jsp
